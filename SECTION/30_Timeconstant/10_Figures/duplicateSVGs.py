...
def parseArguments():
    ...
    parser.add_argument("-d", "--delimiter",
                        help="Specified delimiter in xml", type=str, default=',')
    parser.add_argument(
        "-a", "--attrName", help="Name of the added attribute in the svg file", type=str, default='exercise')
    parser.add_argument(
        "-s", "--suffix", help="suffix that will be added", type=str, default='modLayer')
    parser.add_argument("-o", "--outputFolder", help="folder where files will be stored", type=str,
                        default=os.path.dirname(os.path.realpath(os.getcwd())))
    parser.add_argument("-f", "--workingFolder", help="folder where SVG are", type=str,
                        default=os.path.dirname(os.path.realpath(os.getcwd())))
...
class svgModifier:
    ...
    def __init__(self, argPaser):
        ...
        self.listOfFiles = []
        print('runnig in %s' % (self.workingFolder))
    ...
    def iterateOverCombinations(self, combinations, fileName, tree, layers):
        for combination in combinations:
            ...
            for currentLayer in layers:
                ...
                currentStyle = currentLayer.get('style')
                if currentStyle != None:
                    setValue = re.sub("display:(none|inline)",
                                      setValue, currentStyle)
                currentLayer.set('style', setValue)

            newFileName = fileName[:-4] + '_' + \
                combination + '_' + self.suffix + fileName[-4:]
    ...
    def iterateOverFiles(self):
        for fileName in self.listOfFiles:
            ...
            if combinations is not None:
                self.iterateOverCombinations(
                    combinations, fileName, tree, layers)
