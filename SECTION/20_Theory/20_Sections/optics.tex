...
\begin{figure}[tbp]
  \centering
  % \input{\relPath/10_Figures/TikZ/calibration.tikz}
  \includegraphics[]{External/calibration.pdf}
  \caption{Fast Fourier Transform (FFT) of over 10 cycles averaged Brownian 
    motion (\textcolor{cdata}{blue}) along the $\ex$ direction from a trapped 
    \SiO~particle with of $\SI{1.03}{\um}$. The gray shaded region 
    \SIrange{20}{6000}{\hertz} depicts the data points taken for the fit 
  (\textcolor{cfit}{red}). The sampling frequency of the DAQ system is 
\SI{1.25}{\mega\hertz} and the frequency resolution $\Delta f=\SI{2}{\hertz}$.}
...
By solving \cref{eq:TO-cal-eom} in the frequency domain one finds
\begin{equation}
...
\cname{Werr2019}. However, the material of those sensors were titanium and 
