...
\begin{figure}[tbp]
  ...
  \caption{Schematic of light path from laser head to quadrant photo detectors. 
    In the top half of the sketch red shading and the black lines outline the 
  laser intensity profile qualitatively. In the bottom half just the most outer 
laser rays are depicted.}
  \label{fig:TO-setup}
\end{figure}
...
thesis but is available in~\cite{Lamprecht2017}. An discussion of different 
...
vacuum the speed of light has a smaller magnitude. The refractive index relates 
the speed of light of vacuum $c$ to any other material $c_{\Box}$
\begin{equation}
  c_{\Box} = \frac{c}{n}.
...
parameter with unit \si{\per\meter}. \Cref{fig:TO-n_water} depicts $n$ and 
$\alpha$ of water for different wavelengths $\lal$. Whereas the real part of 
the refractive index is about constant for $\lal > \SI{400}{\nm}$, the 
absorption coefficient changes its magnitude over 4 orders of magnitude in the 
same interval.
...
During the measurements no objects was trapped in the laser focus. They 
...
silica) itself because the absorption coefficient of the fluid 
$\alpha_{\MR{f}}$ is greater than of the particle $\alpha_{\MR{s}}$. Besides 
\cite{Moreau2015} all studies operate at a wavelength of \SI{1064}{\nm}. Our OT 
operates at \SI{785}{\nm} and the absorption coefficient $\alpha_{\MR{f}}$ for 
this wavelength is about one order of magnitude smaller than it is at 
\SI{1064}{\nm},
$\alpha_{\MR{f}}(\SI{785}{\nm}) \approx \SI{2.14}{\per\meter} $ and 
$\alpha_{\MR{f}}(\SI{1064}{\nm}) \approx \SI{15.4}{\per\meter} $ respectively. 
The magnitude of the absorption coefficient $\alpha$ is the measure for how 
much intensity gets absorbed. It contributes linear in the heat equation.
...
\cref{fig:TO-ray_particle} a ray with power $\power{i}{1}$ is incident on the 
...
(see \cref{fig:TO-n_water}) and therefore our maximal convergence angle is 
...
energy $\delta$ inside the particle can be approximated with $N$ discrete 
values
\begin{equation}
...
\begin{equation}
    ...
    \right\}
    \underbrace{\power{i}{m-1}\left( \beta_{n} \right)}_{
...
inside a particle for various radii $R$ and distances $aR$. As 
...
anymore. In addition, we assume a spherical particle and solve the problem with 
a spherical coordinate system (see \cref{fig:TA-coordinate}) with its origin at 
...
\end{equation}
where $P$ is the incident power of the laser and $\delta$ the relative absorbed 
energy of the particle. Finally, the average temperature increase of the 
...
laser power in the low \si{\milli\watt} range.
...
of a measured force with the OT. The parameter that changes the most with a 
...
trapped object within the trap. After a calibration it is also possible to 
convert the movement into the physical unit of meters and then also convert it 
into a force with unit Newton. Besides doing the movement tracking visually 
with, e.g., a high-speed camera, the most common approach is do utilize QPDs.  
The latter is superior to the camera because higher sampling frequencies are 
straightforward achievable, less data is produced during the measurement, as 
well as, a full three dimensional resolution is at hand.
...
linear regime from $-0.15\,R_{\MR{QPD}}$ to $0.15\,R_{\MR{QPD}}$ is rather 
little.  Therefore, it is very important to check and adjust the offset of the 
QPD before each measurement to ensure interpretability of the results.
