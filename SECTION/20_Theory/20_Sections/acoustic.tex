...
\cref{eq:TA-incident-field} is known by the type of incident wave. 
...
type of AS is called \emph{Eckart streaming} named after the person who the 
derived it for the first time~\cname{Eckart1948}. However, this type of AS is 
uncommon in MSAF devices because the fluid is always in some kind of cavity.  
The AS pattern that usually appears in MSAF devices is a combination of 
\emph{Schlichting streaming} at the fluid-structure interface and 
\emph{Rayleigh streaming} in the bulk of the fluid. This interaction of 
...
single nodal pressure plane. In order to visualize those fields, one needs to 
...
\begin{figure}[tbp]
  ...
  \caption{Schlichting and Rayleigh acoustic streaming for an one-dimensional 
    standing pressure wave. In the darker boundary regions with width $\delta$ 
    Schlichting streaming (\textcolor{red}{red arrows}) is present and in the 
    bulk of the fluid Rayleigh acoustic streaming (\textcolor{olive}{olive 
  arrows}).}
...
\Cref{eq:TA-ARFy} is valid for a one dimensional pressure wave. However, it is 
