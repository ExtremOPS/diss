...
\begin{figure}[tbp]
  \centering
  % \input{\relPath/10_Figures/TikZ/deformed_circle.tikz}
...
only collecting terms that are equal or less than first order the mass 
conservation is
\begin{equation}
...
\begin{figure}[tbp]
  \centering
  % \input{\relPath/10_Figures/TikZ/plane_wave.tikz}
...
\end{equation}
where $\delta$ is also known as viscous boundary layer thickness.
...
multipole expansion for a rigid rigid particle has only a dipole contribution; 
for $n=1$, $\vel_{\MR{p}}$ is given by \cref{eq:TA-st-particle-vel}.

For $n=0$ and $n=1$ the constant $\alpha_{0}, \alpha_{1}, \beta_{0},$ and 
$\beta_{1}$ are
\begin{subequations}
\begin{align}
...
\begin{figure}
  ...
  \begin{subfigure}[b]{\textwidth}
    ...
    % \input{\relPath/10_Figures/TikZ/SC_all.tikz}
...
one for $n=1$. Material properties for the particle and the fluid are given in 
\cref{tab:TA-parameters,tab:TC-parameters}. The plotted data is generated with 
the Pyhton module \emph{osaft}~\cite{FankhauserPython2022}.}
  \label{fig:TA-SC}
\end{figure}
...
particle. For our assumption of a spherical particle in a unbounded fluid this 
type of AS is called \emph{Eckart streaming} named after \cname{Eckart1948} who 
the derived it for the first time. However, this type of AS is uncommon in MSAF 
devices because the fluid is always in some kind of cavity. The AS pattern 
that usually appears in MSAF devices is a combination of \emph{Schlichting 
streaming} at the fluid-structure interface and \emph{Rayleigh streaming} in 
the bulk of the fluid. This interaction of streaming fields is named after 
\cname{Schlichting1932} and \cname{Rayleigh1894}, respectively, and 
qualitatively depicted in \cref{fig:TA-acoustic_streaming} for a plane standing 
pressure wave with a single nodal pressure plane. In order to visualize those 
...
\begin{figure}[tbp]
  \centering
  % \input{\relPath/10_Figures/TikZ/acoustic_streaming.tikz}
  ...
  \caption{Schlichting and Rayleigh acoustic streaming for an one-dimensional 
    standing pressure wave. In the viscous boundary layer $\delta$ (darker 
...
\begin{equation}
  ...
  -
  \underbrace{
    \timeaverage{\frac{1}{-\iu\omega}\,\vel_{i}\,\pdv{\vel_{i}}{x_{j}}}
  }_{\text{Stokes drift}}
  \quad\text{at~}r=R.
  \label{eq:TA-nd-BCs}
\end{equation}
...
velocity field. Interested reader are pointed to \cname{Doinikov1994Rigid} 
...
direction of the flow which is know as Stoke's drag force
\begin{equation}
...
\sfrac{\R\,f}{\cfl}$, and the ratio of the viscous boundary layer thickness to 
the acoustic wavelength $\sfrac{\delta}{\wavelength_{\MR{a}}}$.

\begin{figure}[tbp]
  \centering
  % \input{\relPath/10_Figures/TikZ/lambda_mode.tikz}
...
But for most applications, firstly, the particle radius is much larger than 
then viscous boundary layer thickness $R \gg \delta$ and, secondly, the 
acoustic wavelength is much larger than the particle radius 
$\wavelength_{\MR{a}}\gg R \gg \delta$. The most common formula for those 
restrictions and an one dimensional standing pressure wave is the derivation by 
\cname{Gorkov1962}. He derives the ARF on a particle which is much smaller as 
the acoustic wavelength ($\R\ll\lambda_{\MR{a}}$). In his derivations, he 
models the fluid as well as the particle as an inviscid material. Hence, no 
viscous boundary layer can be formed The ARF is defined as the gradient of a 
potential
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:TA-ARFy}
\end{equation}
with the so called acoustic energy density
\begin{equation}
...
displaced into the pressure nodes of the two superimposing pressure waves and 
the particles with negative acoustic contrast factor in the superimposed 
pressure antinodes.

\begin{figure}[tbp]
  \centering
  % \input{\relPath/10_Figures/TikZ/viscous_torque.tikz}
  \includegraphics[]{External/viscous_torque.pdf}
  \caption{Sketch of a two dimensional orthogonal pressure field in a fluid 
    cavity of width $W$ and length $L$ with phase difference $\zeta$. Particles 
    gather in pressure nodes ($\Phi > 0$) along respective direction and 
    non-spherical particles rotate with angular velocity $\Omega\left( \zeta 
  \right)$.}
...
solution to the radiation torque exerted onto the objects~\cite{Lamprecht2017}.
 ...
 \begin{figure}
  ...
  \begin{subfigure}[b]{0.3\textwidth}
    ...
    % \input{\relPath/10_Figures/TikZ/viscous_torque_2.tikz}
...
boundary layer thickness $\delta$ around the particle are high enough; 2) the 
excitation of the two pressure waves is phase shifted.
...
rotational direction is also opposite. We will discuss this kind particle 
